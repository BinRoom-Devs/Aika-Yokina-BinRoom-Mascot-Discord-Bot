...
def init_db():
    ...
    # Migrasi otomatis dari berkas JSON jika database baru/kosong
...
def save_user_chat(user_id:int, messages:list[dict]) -> int|None:
...
def save_all_gd_players(players:list[dict]) -> None:
    with get_connection() as conn:
        ...
        # Hapus pemain yang sudah tidak ada di list
...
def delete_gd_player(account_id_gd:int|None=None, nama_user_gd:str|None=None) -> bool:
    with get_connection() as conn:
        cursor = conn.cursor()
        if account_id_gd:
