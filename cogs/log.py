...
class ServerLogger(commands.Cog):
    ...
    def format_durasi(detik:int) -> str:
        ...
        for name, unit_sec in units[:-1]:
            if detik >= unit_sec:
                val, rem = divmod(detik, unit_sec)
    ...
    def _buat_footer_sistem(self) -> str:
        ...
        return f"Aika Core {version}  •  PID-{pid}:{hex_hash}"
    ...
    async def on_app_command_completion(self, interaction:discord.Interaction, command:app_commands.Command|app_commands.ContextMenu) -> None:
        ...
        pesan_lengkap = f"/{command.qualified_name}{options_str}"[:1000]
    ...
    async def on_app_command_error(self, interaction:discord.Interaction, error:app_commands.AppCommandError) -> None:
        ...
        embed.set_footer(text=f"Error ID: {error_id}  •  Waktu proses: {delta:.2f}ms")
        if self.bot.user:
            embed.set_thumbnail(url=self.bot.user.display_avatar.url)
    ...
    async def on_command_completion(self, ctx:commands.Context) -> None:
        ...
        pesan_lengkap = ctx.message.content[:1000] if ctx.message.content else "*Tidak ada teks*"
    ...
    async def on_command_error(self, ctx:commands.Context, error:commands.CommandError) -> None:
        ...
        if isinstance(error, commands.CommandNotFound):
            ...
            for cmd in self.bot.walk_commands():
                if not cmd.hidden:
                    semua_cmd.append(cmd.qualified_name)
                    semua_cmd.extend(cmd.aliases)
            
            cocok = difflib.get_close_matches(invoked_cmd, semua_cmd, n=1, cutoff=0.6)
            if cocok:
                saran = cocok[0]
                await ctx.send(f"❌ Perintah `{invoked_cmd}` tidak ditemukan.\n\n-# Mungkin maksudmu `{ctx.prefix}{saran}`?")
            else:
                await ctx.send(f"❌ Perintah `{invoked_cmd}` tidak ditemukan.")
            return
    ...
    async def on_message_edit(self, before:discord.Message, after:discord.Message) -> None:
        ...
        embed.add_field(name="Sebelum", value=(before.content or "*Tidak ada teks*")[:1024], inline=True)
        embed.add_field(name="Sesudah", value=(after.content or "*Tidak ada teks*")[:1024], inline=True)
    ...
    async def on_message_delete(self, message: discord.Message) -> None:
        ...
        content = (message.content or "*Tidak ada isi teks*")[:1024]
        ...
        if len(attachments) == 1:
            ...
            att_list = [f"[{att.filename}]({att.url})" for att in attachments[:5]]
            if len(attachments) > 5:
                att_list.append(f"*...dan {len(attachments) - 5} lainnya*")
            embed.add_field(name=f"Lampiran ({len(attachments)})", value="\n".join(att_list), inline=False)
            await channel.send(embed=embed)
        else:
            await channel.send(embed=embed)
    ...
    async def on_invite_create(self, invite: discord.Invite) -> None:
        ...
        inviter_mention = invite.inviter.mention if invite.inviter else "Tidak Diketahui"
        ...
        embed.add_field(name="Dibuat oleh", value=inviter_mention, inline=True)
    ...
    async def on_member_update(self, before:discord.Member, after:discord.Member) -> None:
        ...
        if before.guild_banner != after.guild_banner:
            embed = discord.Embed(
                ...
                description=f"{after.mention} memperbarui foto sampul khusus server ini.",
                color=member_color if member_color.value != 0 else WARNA_AIKA,
                timestamp=discord.utils.utcnow()
            )
            embed.set_author(name=str(after), icon_url=after.display_avatar.url)
            embed.add_field(name="Sebelum", value=f"[Lihat Gambar]({before.guild_banner.url})" if before.guild_banner else "*Global*", inline=True)
            embed.add_field(name="Sesudah", value=f"[Lihat Gambar]({after.guild_banner.url})" if after.guild_banner else "*Global*", inline=True)
            embed.set_footer(text=f"ID User: {after.id}")
            await channel.send(embed=embed)
        ...
        if before.guild_avatar != after.guild_avatar:
            ...
            embed.set_author(name=str(after), icon_url=after.display_avatar.url)
            embed.add_field(name="Sebelum", value=f"[Lihat Gambar]({before.guild_avatar.url})" if before.guild_avatar else "*Global*", inline=True)
            embed.add_field(name="Sesudah", value=f"[Lihat Gambar]({after.guild_avatar.url})" if after.guild_avatar else "*Global*", inline=True)
            embed.set_thumbnail(url=after.display_avatar.url)
            embed.set_footer(text=f"ID User: {after.id}")
            await channel.send(embed=embed)
        ...
        if before.roles != after.roles:
            ...
            if added:
                added_mentions = ", ".join([r.mention for r in added])
                embed = discord.Embed(
                    title="➕🏷️ Role Ditambahkan",
                    description=f"{after.mention} mendapatkan role {added_mentions}.",
                    color=discord.Color.green(),
                    timestamp=discord.utils.utcnow()
                )
                ...
                if "Tidak ada alasan" not in reason:
                    embed.add_field(name="Alasan", value=reason, inline=True)
                if isinstance(actor, (discord.User, discord.Member)) and actor.id != after.id:
                    embed.add_field(name="Diubah oleh", value=actor.mention, inline=True)
                embed.set_footer(text=f"ID User: {after.id}")
                await channel.send(embed=embed)
            
            if removed:
                removed_mentions = ", ".join([r.mention for r in removed])
                embed = discord.Embed(
                    title="❌🏷️ Role Dicabut",
                    description=f"Role {removed_mentions} dicabut dari {after.mention}.",
                    color=discord.Color.red(),
                    timestamp=discord.utils.utcnow()
                )
                embed.set_author(name=after.name, icon_url=after.display_avatar.url)
        ...
        if before.timed_out_until != after.timed_out_until:
            ...
            if after.timed_out_until and (before.timed_out_until is None or after.timed_out_until > before.timed_out_until):
                ...
                if before.timed_out_until <= discord.utils.utcnow():
                    embed = discord.Embed(
            ...
            if before.channel is None and after.channel:  # Join VC
                ...
                if len(before.channel.members) == 0:  # VC empty
                    ...
                    if waktu_mulai_vc:
                        ...
                        str_durasi_vc = f"\n-# Sempat VC selama __{self.format_durasi(total_detik_vc)}__"
        ...
        if before.self_stream != after.self_stream:
            ...
            if after.self_stream:
                ...
                if waktu_streaming:
                    ...
                    str_durasi_strm = f"\n-# (Sempat streaming selama __{self.format_durasi(total_detik_strm)}__)"
            self._dirty = True
    ...
    async def on_guild_channel_create(self, channel:discord.abc.GuildChannel) -> None:
        ...
        title_emoji, desc_emoji = channel_icons.get(channel.type, ("➕", ""))
    ...
    async def on_guild_channel_update(self, before:discord.abc.GuildChannel, after:discord.abc.GuildChannel) -> None:
        ...
        if before.name != after.name:
            ...
            title = "📁✏️ Nama Kategori Diperbarui" if is_cat else "#️⃣✏️ Nama Channel Diperbarui"
            desc = f"Nama kategori **{after.name}** telah diubah oleh {actor_mention}." if is_cat else f"Nama channel {after.mention} telah diubah oleh {actor_mention}."
            
            embed = discord.Embed(title=title, description=desc, color=WARNA_AIKA, timestamp=discord.utils.utcnow())
            embed.add_field(name="Sebelum", value=f"`{before.name}`", inline=True)
            embed.add_field(name="Sesudah", value=f"`{after.name}`", inline=True)
            embed.add_field(name="Alasan", value=reason, inline=True)
    ...
    async def on_guild_role_update(self, before:discord.Role, after:discord.Role) -> None:
        ...
        if before.permissions != after.permissions:
            ...
            if diff_added:
                embed.add_field(name="Izin Ditambahkan", value=", ".join(diff_added)[:1024], inline=True)
            if diff_removed:
                embed.add_field(name="Izin Dicabut", value=", ".join(diff_removed)[:1024], inline=True)
    ...
    async def on_guild_stickers_update(self, guild:discord.Guild, before:list[discord.Sticker], after:list[discord.Sticker]) -> None:
        ...
        embed = discord.Embed(
            title="💮✏️ Stiker Diperbarui",
            description="Terjadi perubahan pada daftar stiker server.",
            color=WARNA_AIKA,
            timestamp=discord.utils.utcnow()
        )
        await channel.send(embed=embed)
    ...
    async def on_scheduled_event_create(self, event:discord.ScheduledEvent) -> None:
        ...
        if event.description:
            embed.add_field(name="Deskripsi", value=event.description[:1024], inline=False)
    ...
    async def on_socket_raw_dispatch(self, msg:dict[str,Any]) -> None:
        ...
        if event_type == "AUTO_MODERATION_ACTION_EXECUTION":
            ...
            if matched_content:
                embed.add_field(name="Konten Terdeteksi", value=f"`{matched_content[:1000]}`", inline=False)
            ...
            embed.add_field(name="Status Baru", value=status_text[:1024], inline=False)
