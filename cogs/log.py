import discord, os, sys, secrets, uuid
...
class ServerLogger(commands.Cog):
    ...
    def cog_unload(self):
        self.bot.loop.create_task(self.kirim_log_shutdown())
    ...
    async def kirim_log_restart(self):
        for guild in self.bot.guilds:
            ...
            if channel:
                embed = discord.Embed(
                    title="🔄🌸 Aika Akan Restart",
                    description="Aika sedang melakukan proses restart.",
    ...
    async def kirim_log_shutdown(self):
        for guild in self.bot.guilds:
            ...
            if channel:
                embed = discord.Embed(
                    title="🛑🌸 Aika Akan Shutdown",
                    description="Sesaat lagi, Aika akan offline.",
                    color=0xDD2E44
                )
                embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.display_avatar.url)
                embed.set_thumbnail(url=self.bot.user.display_avatar.url)
    ...
    async def baca_audit_log(self, guild:discord.Guild, action:discord.AuditLogAction, target_id:Optional[int]=None) -> tuple[str,str]:
        ...
        try:
            async for entry in guild.audit_logs(limit=5, action=action):
                if target_id is None or (entry.target and entry.target.id == target_id):
                    actor = f"{entry.user.mention} (`{entry.user.id}`)"
        ...
        if not self._has_started:
            ...
            for guild in self.bot.guilds:
                ...
                if channel:
                    embed = discord.Embed(
                        title="🟢🌸 Aika Online",
                        description=f"Aika sudah online dan selesai proses inisialisasi.\nSiap melayani server!",
                        color=0x76AE58
                    )
                    embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.display_avatar.url)
                    embed.set_thumbnail(url=self.bot.user.display_avatar.url)
            ...
            for guild in self.bot.guilds:
                ...
                if channel:
                    embed = discord.Embed(
                        title="🔄🌸 Aika Tersambung Kembali",
                        description=f"Sambungan Aika sempat terputus dengan Discord.\nSekarang Aika sudah tersambung kembali dan siap melayani server!",
                        color=0x76AE58
                    )
                    embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.display_avatar.url)
                    embed.set_thumbnail(url=self.bot.user.display_avatar.url)
    ...
    async def on_resumed(self):
        for guild in self.bot.guilds:
            ...
            if channel:
                embed = discord.Embed(
                    title="➡️🌸 Sesi Aika Dilanjutkan",
                    description=f"Aika berhasil melanjutkan sesi gateaway.",
                    color=0x76AE58
                )
                embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.display_avatar.url)
                embed.set_thumbnail(url=self.bot.user.display_avatar.url)
    ...
    async def on_app_command_completion(self, interaction:discord.Interaction, command:discord.app_commands.Command|discord.app_commands.ContextMenu):
        ...
        embed = discord.Embed(
            ...
            description = f"Perintah `/{command.qualified_name}` berhasil dieksekusi di {interaction.channel.mention}.",
            color = WARNA_AIKA
        )
    ...
    async def on_app_command_error(self, interaction:discord.Interaction, error:discord.app_commands.AppCommandError):
        ...
        embed = discord.Embed(
            ...
            description = f"Gagal mengeksekusi perintah `/{cmd_name}` di {interaction.channel.mention}.\n\n{str(error)}",
        ...
        await channel.send(content="<@1524951093560213638>, command Aika ada yang eror 😨", embed=embed)
    ...
    async def on_command_completion(self, ctx:commands.Context):
        ...
        waktu_proses = "N/A"
        if hasattr(ctx, "processing_time"):
            waktu_proses = f"{ctx.processing_time:.2f}ms"
        elif ctx.message and ctx.message.created_at:
            delta = (discord.utils.utcnow() - ctx.message.created_at).total_seconds() * 1000
            waktu_proses = f"{delta:.2f}ms"

        footer_parts = [f"Msg ID: {ctx.message.id}", f"Waktu proses: {waktu_proses}"]

        embed = discord.Embed(
            title = "🌸 Command Aika Dijalankan",
            description = f"Perintah `{ctx.command.qualified_name}` berhasil dieksekusi di {ctx.channel.mention}.",
            color = WARNA_AIKA
        )
    ...
    async def on_command_error(self, ctx:commands.Context, error:commands.CommandError):
        ...
        embed = discord.Embed(
            title = "🌸🛑 Command Aika Eror",
            description = f"Gagal mengeksekusi perintah `{ctx.command.qualified_name if ctx.command else 'Tidak Diketahui'}` di {ctx.channel.mention}.\n\n`{str(error)}`",
            color = discord.Color.red()
        )
        embed.set_author(name=f"Pengguna: {ctx.author.name}", icon_url=ctx.author.display_avatar.url)
        embed.set_footer(text=f"Error ID: {error_id}  •  Waktu proses: {waktu_proses}")
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
        await channel.send(content=f"{ctx.guild.get_member(1524951093560213638).mention}, command Aika ada yang eror 😨", embed=embed)
    ...
    async def on_message_delete(self, message:discord.Message):
        ...
        if message.attachments:
            ...
            content += f"\n\n**Lampiran ({len(message.attachments)}):**\n" + "\n".join(att_list)
        ...
        if message.attachments:
            ...
            if first_att.content_type and first_att.content_type.startswith("image/"):
                embed.set_image(url=first_att.proxy_url or first_att.url)
    ...
    async def on_member_remove(self, member:discord.Member):
        ...
        if actor != "Tidak Diketahui / Otomatis Sistem":
            embed = discord.Embed(
        ...
        for guild in self.bot.guilds:
            if guild.get_member(after.id):
                ...
                if before.display_avatar != after.display_avatar:
                    embed = discord.Embed(
                        title="🖼️ Foto Profil Global Diperbarui",
                        description=f"User {after.mention} mengubah avatar global baru.",
                        color=after.color
                    )
                    embed.set_author(name=str(after), icon_url=after.display_avatar.url)
                    embed.set_thumbnail(url=after.display_avatar.url)
    ...
    async def on_member_update(self, before:discord.Member, after:discord.Member):
        ...
        if hasattr(before, "primary_guild") and hasattr(after, "primary_guild"):
            if (before.primary_guild is None or before.primary_guild.id != after.guild.id) and (after.primary_guild is not None and after.primary_guild.id == after.guild.id):
                ...
                binrum = await self.bot.fetch_guild(BINROOM)
                abin = await self.bot.fetch_user(1524951093560213638)
                embed_tag.set_thumbnail(url=binrum.icon.url)
                
                await channel.send(content=f"{abin.mention} 😳", embed=embed)
    ...
    async def on_stage_instance_create(self, stage_instance:discord.StageInstance):
        ...
        embed = discord.Embed(
            ...
            description=f"Stage {stage_instance.channel.mention} telah dimulai.\n**Topik:** {stage_instance.topic}",
            color=discord.Color.green()
        )
    ...
    async def on_stage_instance_delete(self, stage_instance:discord.StageInstance):
        ...
        embed = discord.Embed(
            ...
            description=f"Stage di {stage_instance.channel.mention} telah dihentikan.",
            color=discord.Color.red()
        )
    ...
    async def on_thread_create(self, thread:discord.Thread):
        ...
        embed = discord.Embed(
            title="🧵➕ Thread Dibuat",
            description=f"{thread.mention} dibuat oleh {creator} di channel {thread.parent.mention}.",
            color=discord.Color.green()
        )
    ...
    async def on_socket_raw_dispatch(self, msg:dict):
        ...
        if event_type == "AUTO_MODERATION_ACTION_EXECUTION":
            ...
            embed.add_field(name="Status Baru", value=status or "*Status Dihapus / Kosong*", inline=False)
            embed.add_field(name="Diubah oleh", value=actor, inline=True)
            embed.add_field(name="Alasan", value=reason, inline=True)
