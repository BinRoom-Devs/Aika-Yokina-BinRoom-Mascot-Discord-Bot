        ...
        async with ctx.typing():
            ...
            if snipe_cog:
                if hasattr(snipe_cog, 'snipes'):
                    ...
                    item_dihapus.append(f"Snipe data ({total_snipes} pesan)")
                
                try:
                    from cogs.new_snipe import CACHE_VALIDASI_CDN
                    cache_size = len(CACHE_VALIDASI_CDN)
                    CACHE_VALIDASI_CDN.clear()
                    item_dihapus.append(f"CDN validation cache ({cache_size} URL)")
                except ImportError:
                    pass
            ...
            if pfp_cog:
                try:
                    ...
                    item_dihapus.append("PFP image processing cache")
            ...
            try:
                ...
                TAMPUNGAN_LOG.clear()
                item_dihapus.append(f"Terminal log buffer ({log_size} baris)")
            except ImportError:
                pass
            ...
            if hasattr(self.bot, '_connection') and hasattr(self.bot._connection, '_cache'):
                try:
                    cache_size = len(self.bot._connection._cache)
                    self.bot._connection._cache.clear()
                    item_dihapus.append(f"Discord internal cache ({cache_size} item)")
                except Exception:
                    pass
            ...
            try:
                if hasattr(sys, '_clear_type_caches'):
                    ...
                    item_dihapus.append("Python type caches")
            except Exception:
                pass
